...
def convert_single_example_extra_features(ex_index, example, label_list, max_seq_length,tokenizer):
...
def file_based_convert_examples_to_features_with_extra_features(examples, label_list, max_seq_length, tokenizer, output_file):
  """Convert a set of `InputExample`s to a TFRecord file."""
...
def file_based_input_fn_builder_extra_features(input_file, seq_length, is_training, drop_remainder):
  """Creates an `input_fn` closure to be passed to TPUEstimator."""
  ...
  name_to_features = {
      ...
      "segment_ids": tf.FixedLenFeature([seq_length], tf.int64),
      "label_ids": tf.FixedLenFeature([], tf.int64),
      "extra_features": tf.FixedLenFeature([4], tf.float32), #Adding extra features
      "is_real_example": tf.FixedLenFeature([], tf.int64),
  }
...
def create_model_extra_features(bert_config, is_training, input_ids, input_mask, segment_ids,extra_features,labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
  model = modeling.BertModel(
      config=bert_config,
      is_training=is_training,
